...
from app.services.analysis_pipeline import resolve_job_inputs as _resolve_job_inputs
...
from app.services.score_engine import WEIGHTS, build_evidence
...
from app.services.storage import remove_path, resolve_allowed_data_path
...
def _set_ai_status(sections: dict[str, Any], status: str, error: str = "") -> None:
    ...
    if status == AI_STATUS_PROCESSING:
        sections["_ai_enhancement_started_at"] = utc_now().isoformat()
        try:
...
def _ai_processing_is_stale(sections: dict[str, Any]) -> bool:
    ...
    try:
        ...
        return True
    return utc_now() - started_at > timedelta(seconds=AI_PROCESSING_STALE_SECONDS)
...
def analyze_path(
    ...
    record = AnalysisRecord(
        organization_id=organization_id_for_user(user),
        user_id=user.id,
...
def list_record_versions(record: AnalysisRecord, db: Session) -> list[dict[str, Any]]:
    ...
    query = db.query(AnalysisRecord).filter(
        AnalysisRecord.root_record_id == root_id,
        AnalysisRecord.user_id == record.user_id,
    )
    if record.guest_session_id:
        query = query.filter(AnalysisRecord.guest_session_id == record.guest_session_id)
    else:
        query = query.filter(AnalysisRecord.guest_session_id.is_(None))
    versions = query.order_by(AnalysisRecord.version_no.asc(), AnalysisRecord.created_at.asc()).all()
    return [
        {
            "record_id": item.id,
...
def apply_recommended_job(
    ...
    *,
    ...
    sections_payload["_selected_job_at"] = utc_now().isoformat() + "Z"
...
def download_rewrite_report_file(record_id: int, request: Request, db: Session) -> FileResponse:
    ...
    safe_path = resolve_allowed_data_path(path)
    return FileResponse(
        safe_path,
...
def download_report_file(report_id: int, request: Request, format: str, db: Session) -> FileResponse:
    ...
    if not Path(report.stored_path).exists():
        if not record:
            raise HTTPException(status_code=404, detail="报告文件不存在。")
        generated = ensure_report_file(record, format)
        report.stored_path = str(generated)
        db.commit()
    path = resolve_allowed_data_path(report.stored_path)
